...
Rolling status: Session 91 closed at 988 / 1000 SIL v3.0. All four ranked items shipped, plus a fifth found by a red rather than by the plan. The session tested the confound S90 disclosed rather than inheriting it, and the confound pointed the opposite way: on a structurally fixed denominator, elite density had gone 0.32% → 4.03% across 12 seasons while the mean and median barely moved. That exposed the gate itself — it measured the rostered league blended with an unbounded free-agent pool, and the blend's −0.073 was exactly the number S90 shipped as proof its own fix worked. Potential is now an actual ceiling rather than a drift coefficient, and a distributional gate reads dispersion and elite density together because they demonstrably fail apart. The distributional residual is left reading `out-of-range` on purpose: both routes to green — tuning the engine, and moving the unsourced ceiling — were refused. Public launch remains independently HOLD with launchReady: false.

Previously — Session 90 closed at 985 / 1000 after zero-centring the club development environment; that fix and its guarantee (league-wide mean tilt 0.0000) are untouched and still pass.
<!-- rolling-status-end -->
