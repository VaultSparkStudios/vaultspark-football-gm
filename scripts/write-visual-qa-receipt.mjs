...
const receipt = {
  ...
  inspection: {
    ...
    findings: [
      "S106 changed exactly one player-facing surface — a release note on public/status.html — and it is NOT in this capture set, which covers game surfaces only. It was verified instead against the live staging origin: https://staging.playfranchisearchitect.com/status.html returns HTTP 200 and serves the note, dated 2026-09-12, including the line telling players their saved franchises are untouched. Recorded this way rather than claimed as capture evidence.",
      "These captures are therefore a regression check on the S104 roster structure rather than evidence for new UI. Inspected directly at 1440px dark: the roster carries both active and practice slots with practice rows offering To Active and active rows offering To PS, a position mix spanning OL, DL, LB, DB, RB, WR, TE, QB, K and P, and cap hits rendered per row — the shipped structure, not a quarterback-and-specialist pile-up.",
