...
- [SIL] When a detector and a writer disagree about what a field means, fix the meaning — do not teach the writer to satisfy the detector. `lastSessionSummary` is read against `currentSession - 1`, and `currentSession` had been left one behind, so the brief replayed S100 for four sessions. Making the writer copy `currentFocus` unconditionally turns the block green immediately and replaces a *visibly* stale summary with an *invisibly* wrong one, which is strictly worse for the only reader it exists to serve.
