...
**Decision 11 — one field, two readers, two meanings: honour the ordering, do not edit the propagated renderers.** `render-startup-brief.mjs` computes `expectedSession = currentSession - 1`, so `currentSession` is the session *about to run*. `render-closeout-board.mjs` reads `status.currentSession` as the session it is *closing out*. Both are correct only under one ordering: render the board while `currentSession` still names the session that did the work, then bump it to `n + 1` as the **final write of closeout**. Nothing enforced that, and S103 did not perform the bump — which is exactly why `lastSessionSummary` could sit at S100 for four sessions: the arithmetic could never be satisfied, so the block was permanently stale and was read as ordinary staleness rather than as a broken invariant. **Rationale:** the tempting fix is to point the board at `sil.session` and remove the ordering dependency. Both renderers are propagated from the control plane, so a local edit would drift on the next propagation and would fix this repo by desynchronising it from every other one. The ordering is honoured and written down at `scripts/check-last-session-summary.mjs` — the file a reader reaches for when the block goes red — and flagged in the handoff as an upstream change worth making. **Pattern:** when two readers disagree about what one field means, the local fix is to satisfy both and document the constraint; changing the meaning belongs where the field is defined.
