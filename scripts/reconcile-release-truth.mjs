...
export function reconcileReleaseTruth({ root, report, expectedRevision = null, checkOnly = false, now = Date.now() }) {
  const statusPath = path.join(root, "context", "PROJECT_STATUS.json");
  const status = JSON.parse(fs.readFileSync(statusPath, "utf8"));
  const validation = validateReleaseEvidence({ report, projectStatus: status, expectedRevision, now });
...
if (isMain) {
  try {
    ...
    if (!input) throw new Error("Usage: node scripts/reconcile-release-truth.mjs --input <report.json> [--expected-revision <sha>] [--check]");
