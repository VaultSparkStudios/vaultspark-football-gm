...
test("saveSessionToSlot stamps integrity metadata", async () => {
  ...
  // The stamp describes the *stored* payload, which may be compressed — that is
  // the point: it verifies the bytes on disk are the bytes we wrote.
  assert.equal(meta.integrity.length, storage.getItem("vsfgm:save:dynasty").length);
});

test("clean round-trip loads the snapshot", async () => {
  const storage = createMemoryStorage();
  const store = createBrowserSaveStore({ storage });
  await store.saveSessionToSlot("dynasty", SNAPSHOT);
...
test("silent corruption that is still valid JSON is detected on load", async () => {
  const storage = createMemoryStorage();
  const store = createBrowserSaveStore({ storage });
  await store.saveSessionToSlot("dynasty", SNAPSHOT);
  ...
  await assert.rejects(() => store.loadSessionFromSlot("dynasty"), /integrity verification/);
});

test("truncated payload is detected on load", async () => {
  const storage = createMemoryStorage();
  const store = createBrowserSaveStore({ storage });
  await store.saveSessionToSlot("dynasty", SNAPSHOT);
...
test("legacy saves without an integrity stamp still load (backward compatible)", async () => {
  const storage = createMemoryStorage();
  ...
  const store = createBrowserSaveStore({ storage });
  const loaded = await store.loadSessionFromSlot("old");
  assert.equal(loaded.currentYear, 2031);
});
