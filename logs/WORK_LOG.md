## 2026-09-03 — Session 98 — First-season payoff, chronological trophies, and focus-safe tabs

Completed the full `/arc` from live repository and game-loop authority. The audit reproduced three defects and shipped all three: an uninitialized phase sentinel discarded the first completed season before the existing reckoning/reward pipeline; Trophy Road destroyed the runtime's cross-season chronology by sorting an eight-game window on week alone; and duplicate keyboard handlers closed the tablet drawer before focusing a tab inside its newly inert subtree.

The release gate later found the public status page still stopped at S96; the S98 note was added, inspected in four desktop/mobile/theme captures, and the corrected exact candidate was promoted across every technical surface. One pure transition predicate now distinguishes initial load, ordinary refresh and real season completion; the first valid completion invokes the existing review exactly once. Trophy Road preserves `GameSession.getRecentBoxScores()` as the single ordering authority. The primary tablist has one declared-orientation keyboard owner, keeps desktop roving focus, and returns tablet focus to the visible drawer toggle. A first boot-gate pass correctly rejected a stale preload graph and a 1,892-byte strict-shell overrun; the graph was simplified rather than the ceiling raised, finishing at 649,716 bytes with 53/58 initial modules and zero lazy leaks.

Verification: canonical Node 1,303/1,303 across six shards; exact-SHA CI workflow 33769176381 green; Playwright 55/55; responsive evidence for 255 game states with 98 retained exact-candidate captures plus four reviewed status-page captures; stable staging 14/14 at deployment `a835d039-bd18-4084-8eec-5a60b81993aa` and artifact `6bdc133a478cfee8a8b19321a5684001b65e3322210e1a090b573785240bc7f4`; hosted public-entry and direct-game performance verified. Production Pages workflow 33770515766 and backend workflow 33770517153 use exact candidate `3cce0a51e82e78625bdfad82f1cbdb19a31c94aa`. Package Trust approved the already pinned Wrangler 4.80.0 before Cloudflare deployment.

Public launch remains HOLD. Technical deployment authorization did not supply Zoho receive/reply-as evidence, SHA-bound public-launch approval, lifecycle reconciliation, external Obelisk relying-party proof, or genuine cohort evidence. CDR reviewed; no new creative direction or DECISIONS entry is owed.
...
**Intent check (§3.0.1): ACHIEVED.** The S92 handoff's `Session Intent — S93` booked no blocking item, asked for a fresh `/audit` against live code, and named two cheap checks. The audit was run against live code, both checks were performed and both are recorded in `phantomsRejected` with their evidence (the `population.rostered` reuse sweep found no defect; the NFL ratings-distribution upgrade has no citable source and was deferred honestly rather than booked as unstartable work). Three ranked items shipped, none deferred. No commit used `--no-verify` or `--no-gpg-sign`, so §3.0.1's bypass audit is empty.

**Where the audit came from.** The S92 handoff booked no blocking item and named two cheap checks. Both were run; neither became a ranked item, and both are recorded in `phantomsRejected` with their evidence. The ranked list instead came from a sweep S90's self-improvement loop had explicitly booked and no session had performed: *"a modifier documented as a differentiator must have a league-wide mean of zero and a test that says so. Two occurrences of this class (S71, S90) is a pattern; sweep for a third before it is found by a symptom."* The sweep found the third occurrence. It is not a constant that drifted — it is a shipped UI panel that let the player write the differentiator by hand.

**The finding, and its exact precedent.** `GameSession.updateStaff` still carries S63's note: the Coaching Staff sheet "used to accept playcalling / development / discipline and write them straight into the simulation, clamped 40-99. That made the Coaching Staff panel a god-mode surface: three number boxes, free, any value." Ability now comes only from hiring through the priced coaching market. **Eight lines below that fix, `updateOwnerState` did the identical thing to `owner.facilities`** — no cost, no cash debit, no build time, no owner approval, no rate limit — and `public/game.html` shipped it as three bare number inputs with no min, max or price. All three are live simulation inputs (training to the S90 development environment at `trainingDivisor: 10`; rehab to injury probability and recovery; analytics to `scoutingWeeklyBonus` and therefore draft reveal). `buildFranchiseEconomics` generates every club in [64, 82]; the panel's legal range was [40, 99]. Franchise authority was checked and is **not** the hole: `/api/owner` correctly 403s for a rival club. The authority seam guards *whose* club you edit; nothing guarded whether the edit was earned.

**Measured, not argued.** Seed 20260307, team BUF, ten seasons, control vs one free click on turn one: the write cost nothing (cash 177M -> 177M), facilities went 70->99, 65->99, 73->99, club mean development tilt went **0.322 -> 2.847** against a clamp ceiling of 3.0 and a measured best-club-in-league advantage of +1.14, and after a decade the maxed club's mean roster overall was 79.73 against the control's 77.10 — **league roster rank 1st against 23rd**.

**Two more defects in the same system.** Gate revenue is `marketSize x ticketPrice x 66,500 x attendanceFactor`, and `attendanceFactor` carried no price term anywhere; `fanInterest` moves only on results, press and GM decisions. Measured one season: price 98 -> 57M, price 450 -> 263M, price 35 -> 20M — a 4.59x price multiple returning 4.61x revenue, with fan interest ending at **97 in all three scenarios**. And the league-wide standard deviation of `owner.facilities.training` measured **exactly 5.66 at seasons 0, 3, 6 and 10 on two independent seeds**: no club had ever changed a facility, so the centre S90's differentiator is measured against was a constant wearing a measurement's clothes.

**The fix.** `src/domain/facilityInvestment.js` prices construction with a pure superlinear cost function — pure so every level in every existing save already has a well-defined price and no snapshot migration is needed, the same trick `coachSalary` uses — plus a three-point-per-wing annual build allowance tracked on `owner.facilityInvestment` with an absent-means-unspent default (deliberately *not* derived from `league.transactionLog`, which truncates at 5,000 entries and would silently re-open the allowance in a long franchise). `updateOwnerState` now returns `facilities-readonly`. `src/domain/ticketDemand.js` gives attendance a bounded linear demand curve measured against the **league's own mean price**, never a literal — constant-elasticity demand was considered and rejected because it has no interior optimum, so the dial would still not be a decision. `src/engine/facilityMarket.js` runs an offseason AI investment round through the same priced domain functions, so there is exactly one cost model in the codebase.

**The defect caught before it shipped.** The first implementation measured mean 71.72 -> 72.97 -> 74.81 and sd 5.658 -> 4.398 -> 3.729 at seasons 0/2/5: every club climbing, none ever falling, a +0.6/season ratchet that reaches the ceiling and zero spread well inside the forty-season franchise this project builds for. That would have deleted the S90 development environment as completely as a constant stub. A purchase price cannot supply the counterforce, because it is paid once out of a cash pile that regrows every season; annual upkeep can, because it is a recurring claim on revenue. With upkeep added the same probe measures sd 5.658 -> 4.015 -> 3.655 at seasons 0/3/8 and minimum club cash 114M -> 77M -> 49M, and the S90 league-wide mean tilt stays 0.00000 at every mark.

**Post-fix verification.** Demand factor at the league centre is exactly 1.000 (the centred-differentiator identity, asserted rather than claimed); gate revenue peaks at 1.409x the league mean, worth 1.092x; pricing at the legal maximum of 450 now returns **0.191x — worse than pricing at the mean**, so the exploit is inverted rather than capped. Reaching the facility ceiling costs 357.8M and takes at least nine league years.

**Coverage.** New `test/session93-owner-capital-authority.test.js` (31 tests) including a negative control that reconstructs the pre-S93 clamped free write and proves it reached a level outside the entire generated league; new `tests-ui/s93-owner-capital.spec.js` (4 Playwright tests, real browser) proving the three number boxes are gone from the shipped page and the priced market renders and functions in their place. `test/feature-pack-v1.test.js` had asserted the defect as a feature (`training: 88` must land) and was corrected to assert the new contract — the same correction S63 would have made to a coaching-ratings assertion.

**One gate did its job and was honoured rather than widened.** The settings island's 15% boot-budget headroom floor went red when the new panel was added inline. Rather than raise the budget, the panel's markup — and the Owner spotlight beside it — moved into `public/lib/facilitiesPanel.js`, pulled in with a dynamic import so nobody who never opens the Owner tab pays for it. Boot budget and browser reachability both green afterwards. The public-truth gate separately caught a real claim drift — the new engine module makes 41, and `landing.html` still said 40 — fixed at source.

**A reporting-layer falsehood found while reading this session's own output.** `scripts/render-audit-md.mjs` read only `audit.preverifiedSkips`, but the sidecar schema every session has written since S89 is `phantomsRejected`. S92 shipped that array empty, so "No rejected candidates were recorded" happened to be true and nobody noticed. This session records three rejected candidates and the rendered public page printed the same sentence — false. The renderer now reads both keys and both shapes; re-rendering the S92 audit produces byte-identical output, so no past artifact changes.

**The innovation-pack scanner false-positived on prose, and the fix was its own declared escape hatch.** Dry-running `node scripts/ops.mjs innovation-pack` — the exact command `test/studio-protocol-smoke.test.js` runs — reported one open candidate at `src/domain/facilityInvestment.js`, because the "unfinished behavior" pattern `/not implemented|\bstub\b/i` matched the words *"a constant stub"* in a doc comment describing the ratchet defect this session had just fixed. With every audit item shipped, that test requires the pack to report "No unclassified live innovation candidates remain", so the studio shard would have gone red on a comment. The scanner already ships the right mechanism for exactly this — a file-level `innovation-pack:ignore`, deliberately scoped to the "unfinished behavior" class — with a comment saying rewording good documentation to satisfy a keyword scan is the wrong trade. Using it beat both alternatives (reword the prose, or loosen the pattern). **A live negative control arrived unasked:** the first version of the opt-out note spelled out the explicit inline marker tokens, and the scanner fired again on *that* — proving the pardon really is scoped to one class and does not blanket-silence the file.

**Write-back completeness (§3.1), stated rather than assumed.** CURRENT_STATE, TASK_BOARD, LATEST_HANDOFF, WORK_LOG, DECISIONS, SELF_IMPROVEMENT_LOOP, TRUTH_AUDIT, PROJECT_STATUS and agent memory all reference S93. `docs/CREATIVE_DIRECTION_RECORD.md` does not exist in this repo (it is not among the surfaces propagated to the public tree) and no founder creative direction was given this session — **CDR reviewed, no entry owed**, recorded here rather than left silent. Repo-specific surfaces whose truth changed were also updated: `docs/AUDIT_2026-08-22_SESSION93.json` (sole source of truth) with its rendered `.md`, `docs/INNOVATION_PACK.md`, `docs/closeout-brief-session93.input.json`, `public/landing.html`'s engine-systems claim, the shard manifest, and the API contract. Per §3.4 the board's `## Now` gained a startable item — the facility-equilibrium gate — so S94 does not open on three externally-blocked rows again.

**A gate caught a real defect in this session's own UI wiring, on the last shard of a full run.** `test/browser-promise-observability.test.js` failed the studio shard 206/207: the two dynamic imports added to `tabSettings.js` used bare `.catch(() => …)`, and this project forbids silent browser promise sinks — a lazily imported panel that fails to load must land in the client diagnostics ledger and stay retryable, not disappear behind a reassuring sentence. Root-fixed by routing both through `observeBackgroundTask`, which then pushed the settings island back over its headroom floor (14.8%), which in turn forced the better shape: both panels now take a uniform `(host, state)` signature so the island carries **one** `mountOwnerPanel` helper instead of two near-identical blocks. Promise observability 0 silent sinks, boot budget green, reachability green, and the four Playwright specs still pass. The gate did not just prevent a defect — it produced less duplicated code than the version that would have shipped without it.

**On the receipt's honesty — three runs, and why the last one is the canonical one.** A first `npm test` was started before several closeout-time fixes were found (the `landing.html` engine count, the audit-renderer phantom bug, the scanner opt-out). It completed `core` at 188/188 and was on `runtime` when the last of those edits landed in `src/domain/facilityInvestment.js`. That edit is a comment and cannot change behaviour — which is exactly the argument not to lean on. Because `core` had already run against the previous bytes, that run was **stopped and discarded rather than reported**. The second run went the full distance and ended `studio: 206/207` on the promise-observability guard above — an honest red, root-fixed rather than accommodated, which invalidated it as a receipt for the same reason. The receipt below is the **third** run, from a tree frozen after that fix. Three suite cycles for one session is a real cost and it is scored down in Process Quality rather than rounded away; the lesson, now in the handoff and in agent memory, is that the seconds-long closeout gates (`check-public-truth`, `check-browser-boot-budget`, `check-browser-promise-observability`, `ops.mjs innovation-pack --dry-run`) belong **before** the fifty-minute suite, not after it.

## 2026-08-19 — Session 92 — A Sourced Elite-Density Baseline, and the Population Bug It Uncovered

Completed the full `/arc` from a clean S91 boundary. Phase 0 triage: clean tree, in sync with origin 0/0, no lock, all ten surfaces current through S91 (write-back-currency probe reported 1.1h-old in-flight debt, well under the 12h threshold — not a cut-off). Session identity fixed at S92 before any write. Genius hit list was exhausted, so `/audit` opened on the single item the S91 handoff booked rather than a fresh sweep.

**The task, and the trap inside it.** S91 shipped `LEAGUE_DISTRIBUTION_TARGET.elite90PlusPctCeiling` as `judgement-not-measured` and instructed S92 to source a real NFL elite-density baseline. It also warned, explicitly, not to close the resulting `watch` by tuning `POTENTIAL_REVERSION_PROFILE.rate` or by moving the ceiling — and left open whether the *generator itself*, not just the ceiling, might be wrong. Sourcing an external anchor and stopping there would have been the shallow read of that instruction.

**The anchor.** New `src/data/nflEliteDensityBaseline.js` derives the ceiling from real, structurally stable NFL honor formats rather than the engine's own behaviour: AP First-Team All-Pro (26 seats — one per position, the tightest honor the league gives) as the ceiling, and the Pro Bowl (88 seats — the broader "very good this season" honor) as the watch line, both divided by the real active-roster population those honors are drawn from: 53 players × 32 clubs = 1,696. That denominator is not a coincidence — it is exactly what this project's own `ROSTER_STRUCTURE.activeLimit` already encodes, so the anchor and the measured population line up by construction rather than a unit-conversion fudge. Documented with explicit honest limits: it is an analogy (a real season-*performance* honor mapped onto a declared talent *rating*), not an identity, which is why the baseline ships as a band (tight floor, loose watch line) instead of a false-precision point, and the honor-slot counts are sourced from the stable structure of the awards rather than a live external feed.

**Checking the population match found the real bug.** Before wiring the new ceiling in, the natural question was whether the population the S91 gate measures against actually matches the population the anchor's honors are drawn from. It did not: `population.rostered` (S91's gated population) is `teamIds.has(player.teamId)` — the active roster (53/club) blended with the practice squad (16/club, S89's `ROSTER_STRUCTURE`). Practice-squad players are structurally ineligible for either real honor. Measured live on two independent seeds across a 10-season decade, the practice squad holds **zero** 90+ players (0/496, both seeds) — so blending it in only ever dilutes the ratio, never inflates it. The exact same 57–58 elite players read **2.6–2.7%** on the blended `rostered` population and **3.4%** on the corrected `activeRosterOnly` population.

**The resolution.** Against the sourced band (1.53% ceiling / 5.19% watch line), the verdict moves from `out-of-range` (S91's self-authored 1.6%/2.4% ceiling compared against the diluted population) to `watch` — reached without touching the reversion rate and without moving the ceiling on judgement, the two routes S91's own handoff had already refused. `src/stats/progressionParity.js` adds `population.activeRosterOnly`/`population.practiceSquad` to the population split (with the same `(player.rosterSlot || "active") === "active"` default convention the rest of the engine already uses), and `buildDistributionReceipt` prefers `activeRosterOnly` with a fallback chain that keeps session91-era fixtures working unchanged.

**One error caught in flight, recorded rather than hidden.** The first attempt at the S91 negative-control fixture reconstructed a "real" pre-fix `activeRosterOnly` reading by ratio from S91's disclosed rostered figures (5.16%) — and it landed within noise of the new 5.19% watch line, on the wrong side of it. Rather than adjust the reconstruction until it crossed the line (exactly the fabricated-precision-to-pass-a-gate failure this project's own standing rule forbids), the fixture was rewritten to use an explicitly synthetic, unambiguously-bad value, with the real evidence carried instead by a live two-seed measurement test. Two other new tests failed on first run for legitimate reasons — a freshly generated league carries no practice-squad players at all (49/club, below the 53-man active floor), so a partition test needed a hand-built fixture instead of `createSession`; and a fallback-path test had over-specified an expected classification that depended on a ceiling comparison unrelated to what the fallback wiring itself proves.

**Verification.** New `test/session92-nfl-elite-density-baseline.test.js` (9 tests) plus updated assertions in `test/session91-potential-reversion.test.js` and `test/realism-career-regression.test.js`; all pass, including a live two-seed decade measurement. Targeted suite green (31/31) before the full canonical `npm test` run. Nothing player-visible changed this session — this was a calibration-authority fix. Public launch remains HOLD and `launchReady` stays false.
