# TRUTH_AUDIT.md — archive

Append-only archive of entries rolled out of the live ledger. Verbatim; newest first. See `context/TRUTH_AUDIT.md` for the working set.
...
## 2026-08-17 — Session 90 progression truth

- **Progression truth:** the engine and its own parity receipt were computing different functions. `expectedDevelopment` in `src/stats/progressionParity.js` models `ageFactor + traitFactor`; the engine ran `ageFactor + traitFactor + developmentBonus`, and that third term was the largest of the three at +0.84 OVR per player per offseason. The receipt was therefore blind by construction to the term it most needed to see, which is how room-level parity could report rooms "on-target" while the league inflated underneath it. Resolved by zero-centring the term rather than by adding a second model of it — a shape declared twice is a shape that drifts.
- **Magnitude truth:** the previously reported +0.228 annual drift understated the problem rather than describing it. The declared curve's league-mean move is −0.885/offseason and the undeclared subsidy was +0.84; the observable was two large errors of opposite sign nearly cancelling. Any future reading of a small drift on this system should be decomposed before it is believed to be small.
- **Measurement-window truth:** removing the subsidy swung a three-season sample to −0.42 before the league reached steady state, while the ten- and twelve-season windows land at −0.073. The generated league's initial age distribution is not the simulation's steady state, so a short window on this system reports a defect a decade does not confirm. The three-season assertion written during this session was replaced rather than retained.
- **Receipt-coverage truth:** every "suite green" claim this project has made was scoped to five of six shards. `npm test` had never executed determinism, the S89 cap-legality proof or the career-realism decade. As of this session `long` is in `DEFAULT_SHARDS` and that gap is closed; the canonical receipt now costs roughly twelve minutes more and covers the tests most likely to be load-bearing.
- **Standing-red truth:** the `realism-career-regression` failure S89 disclosed as pre-existing and declined to force-green is genuinely fixed at its source, not suppressed, re-scoped or re-worded. It passes 3/3 including the decade progression-parity assertion, and it now runs under the canonical command.
- **Surface truth:** the player-facing development outlook would have shipped reporting the tilt derived from the stale literals while the offseason progressed players on the measured ones. Found while wiring the fix, not by a probe. Both now read one centres object and the agreement is gated.
- **Residual truth, unranked and stated:** the league's level is fixed; its shape is not. Elite density rises 0.76% → 3.4% across 12 post-fix seasons. The companion top-100-mean figure has a known confound — the free-agent pool grows 1,720 → 2,648 over the same window, so a larger population's top hundred is higher without any change in distribution — and is deliberately not claimed as evidence on its own.
- **Cost/security truth:** no new dependency, no secret custody change, no paid service. The fix is pure local simulation arithmetic; every probe ran on local seeded execution with no inference spend.
- **Launch truth:** unchanged and still HOLD. `launchReady:false` remains correct pending Zoho delivery/reply-as, SHA-bound founder approval, lifecycle authority reconciliation and external Obelisk relying-party proof. Nothing in this session touches those gates.
...
- **Release identity is exact.** Stable staging passes 14/14, production passes 10/10, CI workflow 33030325197 and promotion workflow 33031371311 are green. Visual, performance, staging, and production authorities agree on source `93b867e010dfe3773116433e5b8068dc894e7c0c` and artifact `c45c4a49a46da9e0438219744b8e42e30fb96125ba8c414881c665fe766783a6`. The public `_health` document is compact; the complete 207-file ledger remains in the deploy manifest used by exact promotion verification. The final responsive artifact records 255 states, all 240 required states, zero runtime errors, and zero failures.
