...
test("the pre-game brief states the opponent read the engine acts on", async ({ page }) => {
  ...
  // S103 — a bye is decided BEFORE the modal now, so it has to be handled here.
  //
  // This branch used to live inside the brief: the modal opened on a bye and
  // said "No controlled-team game". S102 made a bye suppress the tactic step
  // entirely (`composeWeeklyPlan`: `tacticalPhase = regularSeason && !onByeWeek`),
  // so on a bye there is no modal to inspect and the old in-modal branch became
  // unreachable. The league here is generated from a random seed, so whether the
  // controlled team plays in the first advanced week varies per run — measured
  // locally at roughly one bye in five, which is exactly how often this test
  // failed while every other assertion in the file passed. The receipt is the
  // authority the product itself renders, so assert against that.
  const brief = page.locator("#tacticalMatchupBrief");
  const byeReceipt = page.locator(".weekly-plan-receipt", { hasText: "Bye week" });
  await expect(brief.or(byeReceipt).first()).toBeVisible({ timeout: 30_000 });
  if (await byeReceipt.isVisible().catch(() => false)) {
